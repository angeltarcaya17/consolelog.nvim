...
function M.patch(project_root, ws_port)
  debug_logger.log("REACT_PATCH", string.format("Patching React app for port %d", ws_port))
  ...
  if vim.fn.filereadable(inject_script_path) ~= 1 then
    debug_logger.log("REACT_PATCH", "ERROR: inject-client.js not found")
    return false
  end
  ...
  local injection_script = string.format("window.__CONSOLELOG_WS_PORT = %d;\nwindow.__CONSOLELOG_PROJECT_ID = '%s';\n%s %s", ws_port, project_id, sourcemap_content, inject_content)
  ...
  if vim.fn.filereadable(index_html) == 1 then
    ...
    if not content:match("ConsoleLog%.nvim auto%-injection") then
      ...
      debug_logger.log("REACT_PATCH", "Patched public/index.html")
      vim.notify("ConsoleLog: React app patched. Restart dev server.", vim.log.levels.INFO)
      return true
    else
      debug_logger.log("REACT_PATCH", "Already patched")
      return true
    end
  end
  
  debug_logger.log("REACT_PATCH", "No suitable files found to patch")
  return false
end
...
function M.unpatch(project_root)
  ...
  if vim.fn.filereadable(index_html) == 1 then
    ...
    if new_content ~= content then
      ...
      debug_logger.log("REACT_PATCH", "Unpatched index.html")
    end
  end
end
